...
- **A9 Optimal Synergy (Governor decree 2026-07-20)** — in every situation seek the HOLISTIC synthesis where every real source of value is kept and made to serve the platform TOGETHER, rather than defaulting to a this-or-that that discards value. An element may be MORE THAN ONE THING (multi-facet; complementary polarities held together — e.g. a concern with a universal invariant *corespine* facet AND a context-modulated *overlay* facet), each facet in its proper crisp form. A forced binary that throws away real value is a FAILURE, not a simplification. GUARDRAIL (keeps it rigorous, not mush): ORTHOGONAL FACETS — each kept value stays crisp + singly-homed (SSOT/A8); facets complement, never contradict. SSOT: FOUND-00001 A9. (8-Q stress-test to harden.) **Corespine-as-infrastructure (A2 reframe, same decree):** a corespine is the reusable INHERITANCE INFRASTRUCTURE elements USE, not a container everything belongs to — full A2/ARCH-00400 rework parked (IBD-0017).
