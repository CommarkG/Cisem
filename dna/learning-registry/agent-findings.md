...
- [2026-08-08 · cisem-sonnet · GI-68 implementability-lens of ARCH-00433 v1.2] **STEP 1 targets the wrong plans and uses an unrecognized marker format.** build-state.sh was run live: current [BUILD-STATE] findings are for ARCH-00429/430/431 (paths with `...md` abbreviation pattern) — NOT ARCH-00410 or ARCH-00420-PART08, which have no current [BUILD-STATE] findings. Separate issues: (a) ARCH-00410 mindmap.js row already carries `NOT-YET-BUILT` (in UNBUILT_MARKERS) → [BUILD-STATE] PASSES for it already. (b) ARCH-00420-PART08: `.claude/settings.json` EXISTS on disk → build-state.sh never flags it. (c) STEP 1 action format `# unbuilt:` is NOT in the UNBUILT_MARKERS list in build-state.sh (the valid markers are: `NOT-YET-WIRED, NOT-YET-BUILT, implementation is a LATER, follow-on build, awaiting build, buildable, not yet greenlit`) — so even if added, it would not affect build-state.sh behavior. (d) Typo: `settings.js` should be `settings.json`. DoD ("no [BUILD-STATE] findings for ARCH-00410 or ARCH-00420-PART08") is already met without any action. CLASS PREVENTION: when a plan step claims to route a WARN check's findings, verify the check ACTUALLY shows those findings for those targets (run the check before planning, not after) and verify the marker format is in the recognized list. → DISTILL-PENDING

- [2026-08-08 · cisem-sonnet · GI-68 implementability-lens of ARCH-00433 v1.2] **STEP 6 violates node-as-context-carrier (CLAUDE.md Principle 14) — Brain relay content is not embedded in the plan.** STEP 6 says "Contents: the 6 roofs and their constituent items, as received" but the actual Brain relay content is not included in the plan document. The plan's own Self-Compliance section cites "Node-as-context-carrier: each step specifies file + action + DoD without external reference" — a principle STEP 6 violates. A future Sonnet executor reading only the repo would have no content to create this file. CLASS PREVENTION: when a plan step creates a new file whose content comes from an external relay (Brain, Governor, etc.), the content must be embedded INLINE in the plan, or the step must explicitly cite a repo-resident artifact that holds the content. An implementation step that says "as received" is not self-contained. → DISTILL-PENDING

- [2026-08-08 · cisem-sonnet · GI-68 implementability-lens of ARCH-00433 v1.2] **STEP 4 references a `prevention:` field that does not exist in RI-0021.** Read RI-0021 at root-insights.yaml:163-169. Its fields are: date, trigger, root_insight, routes_to, status, recurrence_2026_07_27. No `prevention:` field exists. The v1.0 action "extend the `prevention:` field" is targeting a non-existent field. After v1.2 correction, RI-0021 gets only a `sibling:` cross-reference, but where this new field goes in the YAML structure is ambiguous (after which existing field?). CLASS PREVENTION: a plan step that says "extend the [X] field" must verify the field exists in the target file/entry during plan authoring. This is a structural reference error discoverable by reading the source before writing the plan. → DISTILL-PENDING

- [2026-08-08 · cisem-sonnet · GI-68 implementability-lens of ARCH-00433 v1.2] **build-state.sh has a false-positive class for plans that reference sibling plans using abbreviated filenames (`CISEM-ARCH-00429-...md`).** The path_re `[A-Za-z0-9_./-]*\.md` matches the three-dot abbreviation pattern used in change-logs and GI-68 dual-review sections (e.g. `dna/planning/CISEM-ARCH-00429-...md`). The NNNNN/`....` exclusion in build-state.sh only skips four-dot patterns, not three. Result: 4 false FLAGS in the current run (ARCH-00429, ARCH-00430, ARCH-00431×2). CLASS PREVENTION: the false-positive exclusion `*'....'*` should also cover `*'...'*` (three dots) OR path_re should require the filename segment to not contain consecutive dots. This was not introduced by ARCH-00433 but discovered during this review. → DISTILL-PENDING
- **[MINOR][STEP 2]** Fix bumps "A1–A8" → "A1–A9" but the current axiom set is A1–A12 (CLAUDE.md §2.4). Still-stale after the "fix."

- [2026-08-08 · cisem-sonnet · ARCH-00433 STEPS 2–5 execution] **DISPATCH-VS-PLAN DRIFT: one-click dispatch descriptions were based on ARCH-00433 v1.0 actions, not the ratified v1.4/v1.5 final actions.** Two specific drifts: (A) STEP 2 dispatch said "replace with Axioms A1–A9" but the plan v1.4 HOLD 2 SUPERSEDED this with an unranged pointer "Axioms — see FOUND-00001;". Sonnet correctly followed the plan. (B) STEP 4 dispatch said "find the `prevention:` field" which does not exist in RI-0021 (confirmed in v1.3 EXECUTION GUARD). CLASS PREVENTION: a one-click dispatch is a SUMMARY of a plan at a point in time; when a plan has amendment rounds, the dispatch can silently encode a superseded action. The executor tier MUST read the RATIFIED PLAN DOCUMENT directly (RI-0025 source-not-paraphrase) and treat any dispatch description as a starting hint only. Prevention: the dispatch's STEP descriptions should include the amendment round they reflect (e.g. "v1.4 HOLD 2 action:"). Route: CISEM dispatch authoring protocol / one-click best-practice. → DISTILL-PENDING

- [2026-08-08 · cisem-sonnet · ARCH-00433 STEPS 2–5 execution] **PRE-EXISTING [I16] ZF BLOCKER: `dna/queue/CONSENSUS-PACKAGE-for-Brain-2026-07-26.md` body contains a prose "Status:" occurrence that [I16] false-flags as a status contradiction.** Confirmed: 0 diff vs HEAD (file untouched this session). The body text "Status: 100% manual relay today — CLAUDE.md §1.8:" is not a governance status field — it is prose. The [I16] check uses an xargs pipeline that produces "unmatched quote" warnings and then matches prose "Status:" occurrences. This is a pre-existing false-positive that was somehow clean at HEAD 1d5d763 (per COMPACT STATE ZF claim) but now surfaces. Prevents ZF this run. CLASS PREVENTION: [I16]'s body-status extractor must anchor on YAML-field-level "Status: VALUE" patterns (start-of-line with indent, matching the header format — cf. RI-0012 anchoring class), not any "Status:" substring in prose. Route: plan-audit.sh [I16] calibration improvement, same class as RI-0012. ROUTING for this run: PRE-EXISTING-NOT-INTRODUCED-BY-STEPS-2-5 — route to [I16] check improvement in a future plan. → DISTILL-PENDING

- [2026-08-08 · cisem-sonnet · ARCH-00433 STEP 8 (VOC-00004 creation)] **CLEAN STEP — VOC-00004 created successfully; zero new ZF findings introduced.** File at `dna/vocabulary/CISEM-VOC-00004-concept-definitions.md` with PROVISIONAL-ACTIVE status, correct tags, three definition sections, pocket declaration — all present. [PROPAGATE] section showed "none" (VOC-00004 not a registered SSOT regenerator). NOT ZF result caused entirely by PRE-EXISTING [BUILD-STATE] findings: ARCH-00432 references active-process-router.sh and cisem-status.sh (both absent on disk, no unbuilt-marker). These pre-date STEP 8 and are explicitly in COMPACT STATE as "ARCH-00432 AWAITING-BUILDER-REVIEW." ROUTING for this run: pre-existing ARCH-00432 [BUILD-STATE] findings — route to Opus for ARCH-00432 disposition (separate from STEP 8 scope). **OBSERVATION: [BUILD-STATE] is marked "WARN-only, not in ZF" in plan-audit output but the [ZF] line still reports NOT ZF citing "1 check(s) have open findings above." Script behavior inconsistency — the WARN-only label and the NOT ZF verdict are contradictory. CLASS: script reporting honesty; may warrant a plan-audit.sh clarification pass. → DISTILL-PENDING

- [2026-08-08 · cisem-sonnet · ARCH-00433 STEP 6 (brain-open-queue.md creation)] **CLEAN STEP — brain-open-queue.md created at dna/queue/brain-open-queue.md; zero new ZF findings introduced.** File has BRAIN-ACTIVE status + [queue, brain-channel, coordination] tags (creation-gate PASSES: grep confirms both `Status:` and `tags:` present). All 6 roof Status fields re-derived from ARCH-00433 v1.0–v1.5 amendment state by direct file-read (RI-0025 source-not-paraphrase applied): Roof 1 → PARTIALLY-CLOSED (all 3 ARCH-00433 items FIXED/DONE, verified by reading agent files + plan-audit.sh; 8 follow-ons → ARCH-00435); Roofs 2-6 match task-provided values (IN-PROGRESS / SPLIT-ROUTED / BLOCKING / CLOSED / OPEN — all confirmed against current plan state). NOT ZF is pre-existing ARCH-00432 BUILD-STATE finding, already routed in STEP 8 entry above. STATUS RE-DERIVATION CONFIRMATION: verified STEP 3 (INCOMPLETE-INPUT gate) DONE by reading all 3 agent files; STEP 4 DONE by reading root-insights.yaml RI-0021 status field; STEP 5 DONE by grep on SKILL.md — all consistent with task-provided "DONE" labels. → DISTILL-PENDING (no new class; clean execution)

- [2026-08-17 · cisem-sonnet · P1-P7 consolidation-verification (Fable task)] **ND-01 — `ACTIVE` token in wiring-value-report.sh:42 is NOT in any CISEM enum (ARCH-00011 §4 node/planning/wiring sets).** A check enforcing a phantom status token cannot catch real drift — it validates against a value that no governed file should ever carry. CLASS: every hardcoded status token in a check script must be verified against the machine-readable SSOT enum before the script is called "correct." PREVENTION (routes to P2 plan): machine-readable status-enum.yaml + all consumers read from it; any token not present in the YAML is a build-time error. → DISTILL-PENDING

- [2026-08-17 · cisem-sonnet · P1-P7 consolidation-verification (Fable task)] **ND-02 — plan-audit.sh [TAG-STATUS] (~line 595) mixes node/planning/wiring status tokens in a single unscoped list.** A planning-status value (`IN-PROGRESS`) passes the same gate as a node-status (`RATIFIED`) — the check cannot detect cross-context misuse. CLASS: a status gate that does not know WHICH status context it is validating is not validating anything meaningful. PREVENTION (routes to P2 plan): context-scoped validation requires knowing the file type first, then picking the right enum subset. → DISTILL-PENDING

- [2026-08-17 · cisem-sonnet · P1-P7 consolidation-verification (Fable task)] **ND-03/04/05 — cisem-fable.md and cisem-cic-auditor.md have diverged from the shared agent standing-checks block.** fable is missing INCOMPLETE-INPUT/I22 gate (item 6 of 6). cic-auditor has ZERO MANDATORY STANDING CHECKS section. cic-auditor's HARVEST-BEFORE-RETURN block is also shorter (2 lines missing). CLASS: an agent file is a REGENERATED VIEW of a template; any edit to the template requires re-syncing all consumers — without a generator, any one of 5 agents can silently diverge. PREVENTION: ssot-registry ai_persona.regenerates must be complete (ND-06 fix) + a regen-check script validates block identity. Routes to P5 plan. → DISTILL-PENDING

- [2026-08-17 · cisem-sonnet · P1-P7 consolidation-verification (Fable task)] **TIMING MEASURED (Principle 22): plan-audit.sh = 381s total on this environment.** Dominant contributors: type-match.sh (55s full-tree per-file grep), build-state.sh (~51s), wiring-value-report.sh (~46s), creation-gate.sh (~27s) — all doing repeated whole-tree find/grep without a shared enumeration layer. 15 of 21 check scripts have no standalone planted-test fixture. CLASS: a 6+ minute gate discourages frequent use (the very gate that should fire on every commit). PREVENTION (routes to P1 plan): shared enumerate_governed() + planted-test at each script entry point (Brain guard g1). → DISTILL-PENDING
