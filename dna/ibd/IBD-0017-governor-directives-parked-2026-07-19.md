...
## PENDING SEQUENCE the Governor set (items 1-2-3, in order)
1. **Consolidate + present CONCLUSIONS** once the review set is complete. HAVE: R1 (Claude, IBD-0014), R2 (Gemini, IBD-0015),
   R3 (GPT, IBD-0016). Governor said "this one and three other ones" → a 4th may be coming. ACTION: when Governor says the
   set is complete (or the 4th lands), consolidate all under the Mutual-Titles schema (T1-T6) + convergence/divergence → present conclusions. DO NOT consolidate piecemeal.
2. **Ratify a DRAFT of the pipeline protocol + wizard**, go over ALL vocabulary, align everything. (After conclusions.)
3. **Produce the EXTERNAL-AI ONBOARDING PACKAGE** (item below).

## PARKED ITEMS + ROUTING
...
- **P3 — Knowledge graph (Governor item 7: "all saved inputs must be part of a knowledge graph").** WHAT EXISTS: the repo
  ...
  queryable graph (nodes+typed edges) with traversal. Route: resume ARCH-00382; make every intake (RAW/PURIFIED/archive)
  a typed node with edges (source, supersedes, converges-with, patches). Priority: MEDIUM (ties to R3's typed-graph view).
- **P4 — 70%-token-reduction external solution (Governor: "PARK, urgent — verify it really works, I want to implement").**
