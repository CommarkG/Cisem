...
| I25 | Creation without a ratified plan (A4 unenforced) | this session Opus built I19/I23/I24/ZF/CAL/hooks DIRECTLY, excusing it as "small/safe/dogfooding" — treating the plan as optional | **A plan is MANDATORY (Governor decree 2026-07-18).** NO governed creation (ARCH / protocol / check / agent / skill / axiom / vocab / corespine) may be built without a RATIFIED plan (A4). `legal_kind: RULE`. **PENUMBRA — the ONLY exemptions, explicitly:** IBD raw-capture (capture-first) + SWIFT-safe ZERO-blast extraction (ARCH-00190 §7). **"small / safe / dogfooding" is NOT an exemption.** Enforcement check is itself a plan-item (dogfooded — not built ad-hoc). Binds all tiers |
...
[ ] I25 no governed creation without a ratified plan (A4 MANDATORY) — only IBD-capture + SWIFT-safe zero-blast are exempt; "small/safe/dogfooding" is NOT (Governor decree 2026-07-18)
```
