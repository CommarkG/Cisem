...
**★★ READ FIRST — the Governor-APPROVED design frame for this whole cluster:** `dna/queue/NEWTAB-a-b-c-KNOWLEDGE-GRAPH-SYNTHESIS-2026-07-25.md`. It reframes a/b/c as ONE knowledge-graph (a=substrate/neuronal-grid · c=Threshold=WRITE side/how nodes enter · b=boot=READ side/deterministic load), gives the incremental "generated-manifest-over-existing-files" path (no big-bang rewrite), and the split: **(a) framed here, (b)+(c) build in the fresh tab.** Do NOT re-derive this — it's the RI-0037 fix in action.
**Threshold / Routing / Parking audit** → full self-contained prompt: `dna/queue/NEWTAB-THRESHOLD-ROUTING-PARKING-AUDIT-2026-07-25.md`. **Deeper frame (per the synthesis): not "is the gate wired?" but "how do nodes enter the graph?"**
