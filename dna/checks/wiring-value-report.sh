...
field() {
  local f="$1" label="$2" raw
  raw=$(grep -oiE "(^|[|])[[:space:]]*\*{0,2}${label}:\*{0,2}[[:space:]]*[^|(]*" "$f" 2>/dev/null | head -1)
  printf '%s' "$raw" | sed -E "s/.*${label}:\*{0,2}[[:space:]]*//I" | tr -d '\r' | sed -E 's/\*+//g' | awk '{print toupper($1)}'
}

total=0; live=0; realvalue=0; runs_no_value=0; not_live=0
board=""
...
for d in "${scan_dirs[@]}"; do
  ...
  while IFS= read -r f; do
    ...
    if [ "$ws" = "LIVE" ]; then
      live=$((live+1))
      if [ "$vd" = "YES" ]; then realvalue=$((realvalue+1));
...
echo "   AXIS-2 (did it deliver?): REAL-VALUE (LIVE + value_delivered=yes)=$realvalue · LIVE-but-no-proven-value=$runs_no_value"
if [ "$total" -gt 0 ]; then
