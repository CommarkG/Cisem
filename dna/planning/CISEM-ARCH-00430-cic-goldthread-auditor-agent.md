...
2. The agent's **4-check golden-thread audit** (each references an existing invariant, never redefines it):
   ...
   - **C2 CHAIN-CONTINUITY** (A1 + [I1]/[I5] + CoreSpine — I1 broken forward-cite, I5 broken backward-cite; "Island Detection" is the compound, and C2 needs both since it is bidirectional): each step cites its predecessor AND is cited by its successor — no orphan/broken-link/island.
...
[[CORE-SEED | MUST: cisem-cic-auditor is READ-ONLY (Read/Grep/Glob/Bash, no Write/Edit) — it audits, never fixes/edits/writes a truth-field (I7); findings route to Opus/Sonnet via a plan. HONEST LIMIT (Principle 22): read-only is INSTRUCTION-enforced on Bash, NOT sandboxed — Bash could write via redirects/sed; the agent must NEVER use Bash to mutate any file/index/git state (on integrity until a sandboxed-Bash mechanism exists, RI-0064) | WHY: audit/build separation; an auditor that edits can introduce unratified change; and claiming tool-enforced read-only when it is instruction-enforced is itself a Principle-22 overclaim | VERIFY: agent frontmatter has no Write/Edit; the Bash-never-write instruction is present; no file mutated by an audit run | APPLIES_TO: the cisem-cic-auditor agent, permanently]]
[[CORE-SEED | MUST: each of the 4 checks REFERENCES its existing invariant by name (§3.1/GATE-0 · A1/[I1]/[I5]/CoreSpine · Principle 11/I14/DoD · Principle 18C/I23/Principle 22) — it NEVER coins a new invariant or redefines one | WHY: A8/I10 — one source; a scattered re-definition is the fork this agent exists to catch | VERIFY: every check cites an existing invariant, defines none | APPLIES_TO: the agent's audit protocol]]
