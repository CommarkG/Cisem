...
Searched: `plan-audit.sh` full check list · `dna/checks/definition-of-done.md` · `[I23]` (EXISTS≠ACTIVE, activation-claims on NODES) · `[DOD]` (per-artifact DoD spot-check) · `dna/checks/` dir · ssot-registry. **Finding:** no check verifies that a COMPLETE/RATIFIED PLAN's named DELIVERABLES exist on disk. `[I23]` checks node activation claims; `[DOD]` is a per-artifact spot-check; neither does plan→deliverable existence. **Genuine gap.** `[BUILD-STATE]` EXTENDS the `[I23]`/`[DOD]` existence family (same WARN posture, same `dna/checks/` home), not a new family.
...
7. Print findings under a `[BUILD-STATE]` heading (WARN-only, NOT in the `[ZF]` formula — same posture as `[SEED]`/`[TEMPLATE]`); each finding: `plan → deliverable path (absent, no unbuilt-marker)`. Include the shape-disclosure line (step 6) in the output header.
