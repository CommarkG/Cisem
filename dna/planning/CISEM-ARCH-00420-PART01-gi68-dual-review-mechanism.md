...
2. Extend `[RATIFY-GATE]` in `plan-audit.sh` (today: a RATIFIED plan must cite `stage.?1` AND `sound`) — ADD a second condition, **NO date-auto-detection** (GI-68 design-review fix — a git-touch-date proxy is fragile: any incidental future edit to a legitimately-ratified plan drifts its date forward and false-flags it). Instead, **full-scan EVERY RATIFIED plan** (reuse [RATIFY-GATE]'s existing full-scan loop, pa.sh:632, NOT a date filter): each must contain a `## GI-68 Dual Review` heading (or `dual.review|sonnet.*review|haiku.*review` evidence) OR an explicit `GI-68 exempt (ratified pre-decree)` tag. Missing both → FLAG `PREMATURE-RATIFY: <file> (RATIFIED but no GI-68 dual-review verdict cited)`.
2a. **One-time backfill (build step, replaces the date-inference entirely):** exactly ONE pre-existing plan currently matches `Status:.*RATIFIED` — `dna/planning/CISEM-ARCH-00395-*` (verified live 2026-07-25, git-ratified 2026-07-18, pre-GI-68). At build, add the `GI-68 exempt (ratified pre-decree)` tag to it (Opus stamps — I7). Every future RATIFIED plan carries a real dual-review citation via the /cisem-plan-verify skill, so no further exemptions arise.
3. WARN-only; NOT added to `[ZF]` (same posture as `[RATIFY-GATE]` itself today).
4. **Shape-disclosure (honest boundary):** print each run — `[RATIFY-GATE] verifies a dual-review CITATION-EXISTS on every RATIFIED plan (pre-GI-68 plans carry an explicit exempt tag); it cannot verify review CONTENT was substantive — that is Opus's value-assessment role (Step 4), never a mechanical check.`
...
- **should-FLAG:** synthetic plan `Status: RATIFIED` + Stage-1-sound citation but NO `## GI-68 Dual Review` section and no exempt tag → FLAGGED.
...
## Change log
- v0.1 2026-07-25 (cisem-sonnet, drafted per Opus Core Seed 3 + Brain-consensus'd approach `CISEM-ARCH-00420-items123-approach-consolidated.md` Item #3) — folds Brain's hard-constraint verification (hooks cannot spawn subagents) + the named-not-closed `agent`-type hook future path + the skill+[RATIFY-GATE] resolution.
- v0.2 2026-07-25 (Opus, GI-68 dual-review folded — Sonnet design) — accepted 1 MAJOR: the `[RATIFY-GATE]` extension used a fragile git-touch-date proxy ("dated after 2026-07-23") that drifts forward on ANY incidental future edit → false-flagging a legitimately-ratified plan. DROPPED date-detection entirely: full-scan every RATIFIED plan (reuse [RATIFY-GATE]'s existing loop) + one-time backfill of the exempt tag on the single real pre-GI-68 RATIFIED plan (ARCH-00395). Simpler + no drift. Haiku: 0 protocol gaps. Planning Status → CONSENSUS-REACHED.
