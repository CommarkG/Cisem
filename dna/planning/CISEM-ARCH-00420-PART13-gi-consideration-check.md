...
**Type:** ARCH | **Status:** PROVISIONAL-ACTIVE | **Planning Status:** AWAITING-BUILDER-REVIEW (drafted 2026-07-25 by cisem-sonnet per Opus Core Seed 2, from Brain-consensus'd approach — NOT ratified; Opus soundness + GI-68 dual review + Governor ratification all still pending)
...
2. Create `dna/checks/gi-consideration.sh`:
   ...
   d. **Present** → parse the cited ids (bracket/comma-split); verify EACH resolves: `grep -q "id: <ID>"` against `dna/governor-insights.yaml`, OR `grep -q "^  <ID>:"` against `dna/learning-registry/root-insights.yaml`. **Zero resolve** → FLAG `STALE-CITATION: <file> (wisdom_considered ids do not resolve to any real GI/RI)`. **≥1 resolves** → CLEAN — **presence-not-count** (Brain's anti-over-citation guard: relevance of the cited id is judged by the GI-68 reviewer's sampling pass, Step 5, never demanded as a count here).
...
## Change log
- v0.1 2026-07-25 (cisem-sonnet, drafted per Opus Core Seed 2 + Brain-consensus'd approach `CISEM-ARCH-00420-items123-approach-consolidated.md` Item #2) — folds Brain's presence-not-count + sample-relevance-via-GI-68 resolution + the forward-only retroactivity answer. Planning Status: AWAITING-BUILDER-REVIEW — NOT self-ratified (I7); awaits Opus soundness + GI-68 dual review + Governor decree.
