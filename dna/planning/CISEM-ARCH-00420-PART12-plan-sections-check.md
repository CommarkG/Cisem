...
1. Create `dna/checks/lib/plan-sections-lib.sh` (THREE primitives — GI-68 design-review fix: two booleans could NOT reproduce [TAG-STATUS]'s block-extraction, so a third is required):
   ...
   - `pattern_present <file> <regex>` — a plain **unanchored** `grep -qiE` anywhere in the file. This is BYTE-IDENTICAL to the current `[ROUTING]`/`[ALIGN]`/`[POCKET]` inline greps (verified: pa.sh:541/550 are today unanchored substring matches), so those refactor to it with ZERO behavior change.
   - `heading_block <file> <heading_regex>` — returns the block TEXT between a matched `^## ` heading and the next `^## ` (the stateful `awk` scan `[TAG-STATUS]` (pa.sh:566) already does — GI-68 fix: [TAG-STATUS] extracts+greps a block's CONTENT, which neither boolean can do; this primitive reproduces it exactly + is reusable by any future per-section content check).
   - `heading_present <file> <heading_regex>` — true iff a `^## ` heading matches (ANCHORED). Used ONLY for the 8 brand-new sections (no legacy behavior to preserve → anchoring is a safe improvement there, NOT a regression).
2. Refactor the 4 existing checks to `source` the lib, each to its BEHAVIOR-MATCHING primitive (GI-68 fix — not all are "presence"): `[ROUTING]`/`[ALIGN]` → `pattern_present` (unanchored, byte-identical to their current bare grep); `[TAG-STATUS]` → `heading_block` (block-extract then grep the block, its real logic); `[POCKET]` → `pattern_present` (proxy-field `reasoning_scope`). SAME regex strings, SAME scope. Verify by diffing check output before/after on the current repo state (must be byte-identical except for the new `[PLAN-SECTIONS]` block — this diff is the no-regression proof and would CATCH any accidental anchoring change).
...
## Change log
- v0.1 2026-07-25 (cisem-sonnet, drafted per Opus Core Seed 1 + Brain-consensus'd approach `CISEM-ARCH-00420-items123-approach-consolidated.md` Item #1) — folds RI-0032 (the trigger) + PART08's shared-lib precedent (the pattern reused).
- v0.2 2026-07-25 (Opus, GI-68 dual-review folded — Sonnet design) — accepted 2 MAJORs: (1) added `heading_block()` primitive (2 booleans couldn't reproduce [TAG-STATUS]'s block-extraction); (2) mapped each legacy check to its BEHAVIOR-matching primitive — `pattern_present` (unanchored, byte-identical) for ROUTING/ALIGN/POCKET, `heading_block` for TAG-STATUS, `heading_present` (anchored) only for the 8 new sections (avoids a silent tightening the "plumbing only" claim overclaimed). Haiku: 0 protocol gaps. Planning Status → CONSENSUS-REACHED.
