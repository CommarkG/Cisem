...
| `frontend/js/mindmap.js` or equivalent (NEW, Phase 1, NOT built by this drafting task) | no CISEM-{TYPE}-{SEQ} ID — governed frontend script, per CS-FRONTEND-001 §III pattern | T-SYS, frontend layer | frontend, behavior, mindmap | NOT-YET-BUILT — Phase-2 build; viz-architecture PCR (Mermaid vs pure-JS, §Options) pending before build |
